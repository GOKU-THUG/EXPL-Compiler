...
int codeGen(struct tnode *t, FILE *fptr)
{
    ...
    if (t->nodetype == Function)
    {
        ...
        //after storing the local variables and return value in the registers
        freereg_no=lastreg_local+1;
    ...
    // Nodes which calculate left first
    int left = codeGen(t->left, fptr);

    if (t->nodetype == Variable) // Variable
    {
        int right = codeGen(t->right, fptr);
    ...
    if (t->nodetype == Operator) // Operator:+-*=
    {
        int right = codeGen(t->right, fptr);
