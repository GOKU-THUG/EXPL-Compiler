...
void GInstall(char *name, int type, int size,int rowsize,int colsize,struct Parameter* paramlist) // Creates a symbol table entry.
{
...
void LClear(char* name)
{
    ...
    num_args=0;
    num_var=0;
    printf("Local Symbol table:Function %s freed\n",name);
}
...
int validate(int returntype,char* name,struct Parameter* param)
{
    ...
    if(func!=NULL)
    {
        if(func->type==returntype && strcmp(name,func->name)==0)
        {
