...
Classdef        : Cname '{'DECL Fieldlists MethodDecl ENDDECL MethodDefns '}'       {printf("Classtable:\n");printclasstable(Cptr);Cptr=NULL;}
                ;

Cname           : ID        {Cptr = CInstall($<c>1,NULL);}
                | ID Extends ID {Cptr = CInstall($<c>1,$<c>3);}
                ;
...
AsgStmt :ID EQUAL E 							   {struct tnode* var=makeVariableNode($<c>1);
        ...
        | ID EQUAL NEW '(' ID ')'               {struct tnode* var=makeVariableNode($<c>1);
												 struct tnode* newnode=dynamicmemorynode(var->type,New,NULL,NULL);
												 $$=makeOperatorNode('=',var,newnode);}
        | FIELD EQUAL NEW '(' ID ')'            {
                                                    struct tnode* newnode=dynamicmemorynode($<no>1->type,New,NULL,NULL);
													$$=makeOperatorNode('=',$<no>1,newnode);
                                                }
        ;
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
FIELD     :  SELF '.' ID                            { 
