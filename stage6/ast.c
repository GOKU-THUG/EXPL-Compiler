...
{
    struct Fieldlist* field=FLookup(userdeftype,fieldname); 
...
struct tnode* dynamicmemorynode(struct Typetable* t,int nodetype,struct tnode* left,struct tnode* right)
{
    ...
    //If not userdefined datatype
    if(nodetype!=Initialize && strcmp(t->name,"int")==0 | strcmp(t->name,"bool")==0 | strcmp(t->name,"str")==0 | strcmp(t->name,"void")==0)
    {
...
struct tnode *makeOperatorNode(char c, struct tnode *l, struct tnode *r)
{
    ...
    if(c=='=')
    {
        ...
        if(r->type==TLookup("NULL") && (strcmp(l->type->name,"int")==0 | strcmp(l->type->name,"bool")==0 | strcmp(l->type->name,"str")==0|strcmp(l->type->name,"void")==0|strcmp(l->type->name,"NULL")==0))
        {
            free(temp);
        ...
        if(r->type!=TLookup("NULL") && (l->type!=r->type))
        {
            free(temp);
